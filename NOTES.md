...
| 机器 | 状态 | 说明 |
|---|---|---|
| Linux 主机(bbSoyArch / x86_64 / Arch) | ✅ **已部署并验收 12/12** | `vsay-canto` 可用,能合成能播 |
| 平板 Android 设备(OnePlus Pad 2 / aarch64 / Debian 13 chroot) | ⚠️ **合成 11/12,播放受限** | 装得上、合成正常;播放被 Android 音频 HAL 挡住(§7) |
...
`<内部模块>/backend/macip`(遵循 2026-09-22「设备寻址纪律:MAC 优先」)。
...
正是 `<内部模块>/docs/平板粤语TTS-自播放方案.md` 的「方案 C」。
...
**根因(实测取证,不是推测)**:`<开发场>/dsh-tts/server.js:35` 写死了
...
```bash
<内部模块>/retired/qwen3-tts/restore.sh --dry-run   # 先看
<内部模块>/retired/qwen3-tts/restore.sh             # 真恢复
```
...
`moss-nano-port` 零样本克隆女声,模块 `<开发场>/female-moss-nano-port/`:
...
| 笔记本·样本 | `<内部模块>/docs/samples/A_平板现用_…_F0-235Hz.wav` | 208K |
...
- `<开发场>/dsh-tts/`:注释订正(它**从来不依赖**该模型)+ 新增 `DSH_TTS_VOICE` 音色开关(默认不变)
- `<开发场>/android-voice-ime/`:女声路径改走 chroot `moss-nano-port`;`VitsTtsEngine` → `SherpaTtsEngine` 且**去模型绑定**
- `<内部仓库>/tts-models/`:`MANIFEST.json` 移入 `retired[]`;`SHA256SUMS` 删行;布局文档改例

### 保留(有意为之)

- **A/B 证据**:`<开发场>/female-moss-nano-port/out/DEMO/对照_旧VITS女声.wav` + `RETIRED-旧VITS-证据.md`(带许可警告:仅自用)
...
# §L1b · 2026-09-22 晚:Android 设备(平板 chroot)部署记录与坑
...
词表与工具在 **voice-tts 模块**:`<内部模块>/backend/p2y.py`
(设计/规模/过改检测见 `voice-tts/backend/README-p2y.md`)。

## 13.2 为什么**不能**改 vsay-canto,而必须挂 dispatcher

`vsay-canto` 与Android 设备**字节一致**(md5 `8071d81b02518ab8115b6e4e3d91d22e`)是硬约束 ——
...
| # | 文件 | 位置 | 覆盖 |
|---|---|---|---|
| 1 | 本机 `/usr/local/bin/vsay`(源码 `voice-tts/bin/vsay`) | 补标点之后、切段之前 | 本机 CLI + DSH /speak |
| 2 | Android 设备 chroot `/usr/local/bin/vsay`(源码 `moss-nano-port/tablet-dsh-tts/vsay`) | 同上 | 平板的 DSH /speak |
| 3 | `<开发场>/dsh-tts/server.js` | `speak()` 与 `synthesizeToFile()` | **两条路都兜**(含 /speak-audio) |
...
1. **本机 `/usr/local/bin/vsay` 的 p2y 段尚未落盘** —— 需要 root,`sbrun` 两次都
   "120s 内无人批准"。**但 DSH 的 🔊 已经完全生效**(server.js 那条路覆盖了两条出声路径)。
   落盘方式(二选一):`sbrun` 批准一次;或用 root 跑
   `<内部模块>/install.sh`(它已包含 p2y)。
